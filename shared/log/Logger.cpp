...
void Logger::GetCurrentDateTime(char str[])
{
...
void Logger::printString(const char* typeStr, const char* fmt, va_list argptr)
{
	...
	printString(m_DefaultLogFile, typeStr, fmt, argptr, time);
	
	vector<LogFile_ptr>::iterator begin = m_Observers.begin();
    vector<LogFile_ptr>::iterator end = m_Observers.end();
    while (begin != end)
	{
		printString(*begin, typeStr, fmt, argptr, time);
		begin++;
	}
}

void Logger::printString(LogFile_ptr file, const char* typeStr, const char* fmt, va_list argptr, char *time)
{
	char ptime[45];
